    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-3xl border border-slate-100 shadow-md">
        <div>
          ...
          <p className="text-slate-500 text-xs">Explications didactiques pour chaque point de contrôle.</p>
        </div>
      ...
      <div className="space-y-4 max-h-[60vh] overflow-y-auto custom-scrollbar pr-1">
        {questions.map((q, index) => {
          ...
          const chosenText = userAns ? q.options[userAns.chosenIndex] : 'Aucune réponse'
          ...
          return (
            ...
            >
              <div className="flex justify-between items-start gap-4">
                ...
                <span className={`px-2.5 py-0.5 rounded-full text-xs font-bold ${isCorrect ? 'bg-emerald-100 text-emerald-800' : 'bg-rose-100 text-rose-800'}`}>
                  {isCorrect ? '✓ Correct' : '✗ Erreur'}
                </span>
              </div>
              <h4 className="font-bold text-slate-900 text-sm md:text-base">Question {index + 1} : {q.question}</h4>
              <div className="space-y-2 text-xs md:text-sm">
                <div className="flex items-start space-x-2">
                  <span className="font-semibold text-slate-500 min-w-[130px]">Votre réponse :</span>
                  <span className={isCorrect ? 'text-emerald-700 font-semibold' : 'text-rose-700 font-semibold'}>{chosenText}</span>
                </div>
                {!isCorrect && (
                  <div className="flex items-start space-x-2">
                    <span className="font-semibold text-slate-500 min-w-[130px]">Réponse attendue :</span>
              ...
              <div className="bg-white p-4 rounded-xl border border-slate-100 text-xs md:text-sm text-slate-600 leading-relaxed">
                <span className="font-bold text-emerald-600 block text-[11px] uppercase tracking-wider mb-1">L'explication :</span>
                <p>{q.explanation}</p>
              </div>
            </div>
          )
