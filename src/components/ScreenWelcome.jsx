...
export function ScreenWelcome({ meta, onJoin, onCreateSession, leaderboard, onAdmin, quizList, selectedQuizId, onSelectQuiz }) {
  ...
  const [mode, setMode] = useState(null) // null | 'solo' | 'live'
      ...
      {!mode ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 animate-fade-up" style={{ animationDelay: '0.08s' }}>
          ...
          <button
            ...
            className="group bg-white hover:bg-emerald-50 border-2 border-slate-200 hover:border-emerald-400 rounded-3xl p-7 text-left transition-all shadow-md hover:shadow-xl hover:shadow-emerald-100/60 space-y-3"
          >
            <div className="w-14 h-14 bg-emerald-100 group-hover:bg-emerald-200 rounded-2xl flex items-center justify-center transition-colors text-2xl">
              🎓
            </div>
            <div>
              <p className="font-extrabold text-slate-900 text-lg leading-tight">Entraînement Solo</p>
              <p className="text-slate-500 text-sm mt-1 leading-relaxed">Testez vos connaissances à votre rythme. Résultats et corrections immédiates.</p>
            </div>
          ...
          <button
            ...
            className="group bg-white hover:bg-violet-50 border-2 border-slate-200 hover:border-violet-400 rounded-3xl p-7 text-left transition-all shadow-md hover:shadow-xl hover:shadow-violet-100/60 space-y-3"
          >
            <div className="w-14 h-14 bg-violet-100 group-hover:bg-violet-200 rounded-2xl flex items-center justify-center transition-colors text-2xl">
              📡
            </div>
            <div>
              <p className="font-extrabold text-slate-900 text-lg leading-tight">Session de Classe</p>
              <p className="text-slate-500 text-sm mt-1 leading-relaxed">Rejoignez la session de votre professeur via QR code ou code à 6 lettres.</p>
            </div>
        ...
        <div className="bg-white rounded-3xl border border-slate-100 shadow-xl shadow-slate-200/50 overflow-hidden animate-fade-up" style={{ animationDelay: '0.08s' }}>
          ...
          <div className={`flex items-center justify-between px-6 py-4 border-b border-slate-100 ${mode === 'solo' ? 'bg-emerald-50' : 'bg-violet-50'}`}>
            <div className="flex items-center gap-3">
              <span className="text-2xl">{mode === 'solo' ? '🎓' : '📡'}</span>
              <div>
                <p className={`font-extrabold text-sm ${mode === 'solo' ? 'text-emerald-800' : 'text-violet-800'}`}>
                  {mode === 'solo' ? 'Entraînement Solo' : 'Session de Classe'}
                </p>
                <p className="text-xs text-slate-500">
                  {mode === 'solo' ? 'À votre rythme, corrections immédiates' : 'Synchronisé avec votre professeur'}
                </p>
              </div>
            </div>
            <button
              onClick={() => switchMode(null)}
              className="text-slate-400 hover:text-slate-600 transition text-xs font-semibold flex items-center gap-1 py-1.5 px-3 rounded-xl hover:bg-white"
            >
          ...
          <div className="p-6 md:p-8 space-y-5">
            ...
            {/* Live only — room code FIRST (most important) */}
            {mode === 'live' && (
              <div className="space-y-2">
                ...
                <input
                  ...
                  autoFocus
                  className="w-full px-4 py-4 bg-slate-50 border-2 border-violet-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 font-black uppercase tracking-[0.3em] text-violet-700 transition text-center text-2xl"
                />
                <p className="text-xs text-slate-400 text-center flex items-center justify-center gap-1.5">
                  <span>📺</span>
                  <span>Code affiché sur l'écran de votre professeur — ou scannez son QR code</span>
                </p>
              </div>
            )}
            ...
            <button
              onClick={handleJoin}
              className={`w-full text-white font-bold py-4 px-6 rounded-2xl shadow-lg transition-all flex items-center justify-center gap-2 group
                ${mode === 'solo'
                  ? 'bg-emerald-600 hover:bg-emerald-500 active:bg-emerald-700 shadow-emerald-600/20 hover:shadow-emerald-500/30'
                  : 'bg-violet-600 hover:bg-violet-500 active:bg-violet-700 shadow-violet-600/20 hover:shadow-violet-500/30'
                }`}
            >
              <span>{mode === 'solo' ? 'Commencer l\'entraînement' : 'Rejoindre la session'}</span>
