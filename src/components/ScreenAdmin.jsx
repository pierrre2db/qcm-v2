...
import { ajouterQuiz, supprimerQuiz, mettreAJourQuiz, chargerQuizParId } from '../lib/firestore'
...
export function ScreenAdmin({ quizList, onQuizAdded, onQuizDeleted, onBack }) {
  ...
  // Tabs: 'library' | 'editor' | 'import'
  const [tab, setTab] = useState('library')
  const [editorData, setEditorData] = useState({ rawData: null, editingId: null })
  ...
  async function handleEditorSave({ rawData, title, questionCount, editingId }) {
    if (editingId) {
      await mettreAJourQuiz(editingId, rawData, title, questionCount)
      onQuizAdded({ id: editingId, title, questionCount }) // refresh list
    } else {
      const id = await ajouterQuiz(rawData, title, questionCount)
      onQuizAdded({ id, title, questionCount })
    }
    setTab('library')
    setEditorData({ rawData: null, editingId: null })
  }

  async function handleEditQuiz(quizId) {
    const doc = await chargerQuizParId(quizId)
    setEditorData({ rawData: doc?.rawData ?? null, editingId: quizId })
    setTab('editor')
  }
      ...
      <div className="bg-white rounded-3xl border border-slate-100 shadow-md overflow-hidden">
        <div className="grid grid-cols-3 border-b border-slate-100">
          {[
            ...
            { id: 'import',  label: '📥 Importer JSON' },
          ].map(t => (
            <button
              key={t.id}
              onClick={() => {
                setTab(t.id)
                if (t.id === 'editor' && !editorData.editingId) setEditorData({ rawData: null, editingId: null })
              }}
              className={`py-3.5 px-4 font-bold text-sm transition border-b-2 ${
                tab === t.id
        ...
        <div className="p-6">
          ...
          {tab === 'library' && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="font-extrabold text-slate-900">Quiz disponibles ({quizList.length})</h3>
                <button
                  onClick={() => { setEditorData({ rawData: null, editingId: null }); setTab('editor') }}
                  className="flex items-center gap-1.5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-sm px-4 py-2 rounded-xl transition shadow"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              {quizList.length === 0 ? (
                <p className="text-sm text-slate-400 text-center py-8">Aucun quiz. Créez-en un avec l'éditeur !</p>
              ) : (
                    ...
                    <div key={q.id} className="flex items-center justify-between bg-slate-50 border border-slate-100 rounded-2xl px-5 py-4">
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className="bg-blue-50 hover:bg-blue-100 text-blue-600 font-bold p-2.5 rounded-xl transition"
                          title="Modifier"
                        >
                          <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          {tab === 'editor' && (
            <QuizEditor
              initialRawData={editorData.rawData}
              editingId={editorData.editingId}
